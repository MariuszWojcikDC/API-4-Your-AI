...
SELECT
      *
from
      sys.vector_indexes CREATE VECTOR INDEX idx_text_vector_cosine ON dbo.pubmed_article_chunk_vector (vector) WITH (METRIC = 'cosine');

GO
      ------------------------------------------------------------
      -- Generate embedding for user query ("burnout syndrome")
      ------------------------------------------------------------
      DECLARE @raw_vector NVARCHAR(MAX) = (
            SELECT
                  CAST(
                        AI_GENERATE_EMBEDDINGS('burnout syndrome' USE MODEL BielikLocal) AS NVARCHAR(MAX)
                  )
      );
...
SET
      @query_vector_Bielik = (
            SELECT
                  CAST(
                        '[' + STRING_AGG(CAST(value AS NVARCHAR(MAX)), ',') WITHIN GROUP (
                              ORDER BY
                                    CAST([key] AS INT)
                        ) + ']' AS VECTOR(1998)
                  )
...
FROM
      VECTOR_SEARCH (
            TABLE = dbo.pubmed_article_chunk_vector AS v,
            -- use _vector for BielikLocal or _vector_2 for AzureTextEmbeddingSmall (text-embedding-3-small)
            COLUMN = [vector],
...
------------------------------------------------------------
DECLARE @query_vector_Azure VECTOR(1536) = AI_GENERATE_EMBEDDINGS(
      'burnout syndrome' USE MODEL AzureTextEmbeddingSmall
);
...
SELECT
      ...
      pac.text_chunk
FROM
      dbo.pubmed_article_chunk_vector_2 AS v -- swap to pubmed_article_chunk_vector for BielikLocal
      JOIN dbo.pubmed_article_chunk AS pac ON pac.id = v.id
      JOIN dbo.pubmed_article AS ap ON ap.id = pac.article_id
ORDER BY
      VECTOR_DISTANCE('cosine', @query_vector_Azure, v.[vector]);
